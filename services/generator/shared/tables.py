...
def bom_table_markdown(schema: dict) -> str:
    ...
    for r in (schema.get("bom") or []):
        ...
        if not ((pn or typ) and qty > 0): continue
        key = (pn.lower(), desc.lower(), qty, typ.lower())
        if key in seen: continue
        seen.add(key); rows.append((pn or "-", desc or "-", qty, typ or "-"))
    if not rows: return ""
    lines = ["| Part Number | Description | Qty | Type |", "| --- | --- | :---: | --- |"]
    for pn, desc, qty, typ in rows:
